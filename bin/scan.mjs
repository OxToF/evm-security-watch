...
function renderHtml(meta, deps, hygiene, source, classes) {
  ...
  const depCards = nAdv
    ? deps.advisories.map((a) => `<div class="adv"><span class="chip" style="background:${sevBg(a.severity)}">${esc(a.severity)}</span><div class="adv-body"><a class="adv-id" href="${esc(a.url)}">${esc(a.id)}</a><div class="adv-sum">${esc(a.summary)}</div><div class="adv-pkg">Affects: ${esc(a.packages.join(", "))}</div></div></div>`).join("")
    : `<div class="clean">✓ &nbsp;No advisory affects the exact npm versions pinned in your lockfile.</div>`;
  ...
  const hygieneRows = `
    ...
    <div class="hyg"><span class="hyg-badge ${hygiene.oldPragma ? "warn" : "good"}">${hygiene.oldPragma ? "⚠" : "✓"}</span><div><b>Solidity pragmas</b><div class="muted">${esc(hygiene.solc.join(", ") || "not detected")}${hygiene.oldPragma ? " — a pragma allows &lt; 0.8 (no built-in overflow checks; class #4)." : ""}</div></div></div>`;
...
<div class="doc">
  <div class="hd">
    ${WATCHDOG_LOGO}
    <div><div class="wm">SOLANA <span class="g">WATCHDOG</span></div><div class="tl">EVM &amp; Solidity security scan</div></div>
  ...
  <div class="sub">
    ...
    <div class="pills"><span class="pill">${source.totalFiles} Solidity files scanned</span><span class="pill warn">Hygiene scan · not an audit</span></div>
  </div>
  <div class="body">
    <div class="stats">
    ...
    <p class="muted">Matched against the exact npm versions pinned in your lockfile.</p>
    ${depCards}
    <h2>Build hygiene</h2>
    ${hygieneRows}
    ...
    <div class="ft"><span>${WATCHDOG_LOGO.replace('width="46" height="46"', 'width="22" height="22"')}</span><div>Generated by <b>Solana Watchdog</b> · <a href="https://github.com/OxToF/evm-security-watch">open source</a>. Want continuous coverage instead of a snapshot? Ask about the monthly watch.</div></div>
  </div>
</div>
</body></html>`;
}
