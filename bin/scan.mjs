...
function renderReport(meta, deps, hygiene, source, cta = null) {
  ...
  const fixAll = fixMailto(cta, meta, `Fix request: ${meta.owner}/${meta.repo}`, [`On-chain advisories: ${deps.buckets.onchain.length}`, `Code leads: ${[...source.byClass.values()].reduce((s, e) => s + e.total, 0)}`]);
  ...
  return { md: md.join("\n"), html: renderHtml(meta, deps, hygiene, source, classes, cta) };
}
...
function renderHtml(meta, deps, hygiene, source, classes, cta = null) {
  ...
  const card = (a, onchain = false) => {
    const link = onchain ? fix(`Fix request: ${meta.owner}/${meta.repo}: ${a.id}`, [`Advisory: ${a.id} (${a.severity})`, `Affects: ${a.packages.join(", ")}`]) : null;
    return `<div class="adv"><span class="chip" style="background:${sevBg(a.severity)}">${esc(a.severity)}</span><div class="adv-body"><a class="adv-id" href="${esc(a.url)}">${esc(a.id)}</a><div class="adv-sum">${esc(a.summary)}</div><div class="adv-pkg">Affects: ${esc(a.packages.join(", "))}</div>${link ? `<a class="fixlink" href="${esc(link)}">Get this fixed &rarr;</a>` : ""}</div></div>`;
  };
  const depCards = cov.noExternalImports
    ...
    : nAdv ? bk.onchain.map((a) => card(a, true)).join("")
  ...
  const toolCards = bk.toolchain.length ? bk.toolchain.map((a) => card(a)).join("") : `<p class="muted">None.</p>`;
...
export async function runScan(opts = {}) {
  const { repoUrl, localPath = null, out = "scan-out", fetchImpl = globalThis.fetch, now = new Date(), log = console.log, token = process.env.GITHUB_TOKEN || null, cta = null } = opts;
  ...
  const { md, html } = renderReport(meta, depResult, hygiene, source, cta);
